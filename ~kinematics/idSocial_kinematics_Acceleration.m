function input_data=idSocial_kinematics_Acceleration(input_data,options,plot_mode)
% Calculates the acceleration of each individual.
%
...
if nargin >= 1
    if (isfield(opt(1),'edges') && ~isempty(opt(1).edges) && all(isfinite(opt(1).edges))) || (isfield(opt(1),'limits_min') && ~isempty(opt(1).limits_min) && isfinite(opt(1).limits_min) && ...
        ...
        plot_mode_def.xlabel = 'Acceleration (BL/s^2)';
        ...
        def_options(1).act_method='kinematics_Acceleration4Statistics';
        plot_mode_def.label = 'Acceleration (BL/s^2)';
...
functionInfo.handle=@idSocial_accelerationDistribution;
...
functionInfo.output2function={''; ...
    ...
    'kinematics_Acceleration4Statistics';...
    'kinematics_AccelerationInfo';...
    };
