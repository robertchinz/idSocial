function input_data=idSocial_kinematics_DistanceFromCenter(input_data,options,plot_mode)
% Calculates the distance of each individual to the center of the arena.
%
...
def_options(1).distance_unit={'pixel','bodylength','arena_radius'};
def_options(2).distance_unit='Select units';
...
if nargin >= 1
    if (isfield(opt(1),'edges') && ~isempty(opt(1).edges) && all(isfinite(opt(1).edges))) || (isfield(opt(1),'limits_min') && ~isempty(opt(1).limits_min) && isfinite(opt(1).limits_min) && ...
        ...
        plot_mode_def.xlabel = 'Distance from center';
        ...
        def_options(1).act_method='kinematics_DistanceFromCenter4Statistics';
        plot_mode_def.label = 'Distance from center';
...
functionInfo.output2function={''; ...
    '';...
    '';...
    'kinematics_DistanceFromCenter';...
    'kinematics_DistanceFromCenter4Statistics';...
    'kinematics_DistanceFromCenterInfo';...
    };
%% 
