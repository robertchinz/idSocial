function input_data=idSocial_kinematics_Turning(input_data,options,plot_mode)
...
% movmement (See also idSocial_dynamics_TurningReaction).
%
...
def_options(1).method={'histabs','hist','ksdensity_gauss','ksdensity_epanechnikov','ksdensity_triangular'};
...
if nargin >= 1
    if (isfield(opt(1),'edges') && ~isempty(opt(1).edges) && all(isfinite(opt(1).edges))) || (isfield(opt(1),'limits_min') && ~isempty(opt(1).limits_min) && isfinite(opt(1).limits_min) && ...
        ...
        plot_mode_def.xlabel = 'Turning (BL/s^2)';
        plot_mode_def.ylabel = 'Counts';
       
    else
        plot_mode_def.statistics = {'Mean','Median','Meanabs','Pool'};
        plot_mode_def.display_mode='plot2d';
        def_options(1).act_method='kinematics_Turning4Statistics';
        plot_mode_def.xlabel = 'Turning (BL/s^2)';
    end
end
if nargin == 1
...
functionInfo.handle=@idSocial_turningDistribution;
...
functionInfo.output2function={''; ...
    'kinematics_TurningMedian';...
    'kinematics_TurningMode';...
    'kinematics_Turning';...
    'kinematics_Turning4Statistics';...
    'kinematics_TurningInfo';...
    };
