function figure_windows = idSocial_plotUI(input_data,funcstring,act_set,control,figure_windows)
...
if nargin < 5 || isempty(figure_windows) || ~ishghandle(figure_windows)
    figure_windows = []; 
end
...
for k=1:size(fn,1)
    if ~isempty(strfind(fn{k},'Fig'))
        maxFig = max(maxFig,str2double(strrep(fn{k},'Set','')));
    end
end
act_fig = maxFig+1;
...
if  strcmpi(dm,'dynamicMapPolar') 
    figure_windows = figure('Units','normalized','Position',[.45 .3 .5 .5],'Name','Plot Results','NumberTitle','off','Color','w');
