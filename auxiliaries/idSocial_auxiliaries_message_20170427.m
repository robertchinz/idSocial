function [fh, textBox] = idSocial_auxiliaries_message(help_comment,textBox) 
...
if nargin<1
    ...
    set(textBox,'Tag','idSocialMessageBox');
...
elseif ~isempty(textBox) && ishghandle(textBox)
    try
        ...
        if iscell(actString)
            actString =  textwrap(textBox,vertcat(actString,{help_comment}));
    ...
    panelH = get(get(textBox,'Parent'),'Parent');
    panel2H = get(textBox,'Parent');
    ...
    set(textBox,'Units','normalized','Position', [0 0 1 1])
    set(textBox,'Units','pixels')
