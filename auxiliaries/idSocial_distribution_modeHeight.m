function [mode_loc, hitemp]=idSocial_distribution_modeHeight(vec,edges,bandwidth,method,location)
...
if ~isempty(vec) && ~all(isnan(vec))
    if size(vec,1) == 1 && size(vec,2)>1
        ...
        hitemp = idSocial_auxiliaries_kerneldensity(vec,edges,bandwidth,method);
       
        if isempty(location)
            [~,locs] = findpeaks(hitemp,'sortstr','descend','npeaks',1);
            mode_loc = hitemp(locs);
        elseif strcmpi(location,'first')
            [~,locs] = findpeaks(hitemp);
            mode_loc = hitemp(min(locs));
        elseif strcmpi(location,'last')
            [~,locs] = findpeaks(hitemp);
            mode_loc = hitemp(max(locs));
        end
