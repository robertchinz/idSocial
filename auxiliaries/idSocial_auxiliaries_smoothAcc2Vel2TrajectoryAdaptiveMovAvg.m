function [tr_smooth, tol_out] = idSocial_auxiliaries_smoothAcc2Vel2TrajectoryAdaptiveMovAvg(trajectories,tol_frame,filter_outliers)
...
for ff = 1:no_fish
    ...
    while (all(isinf(dist2o(:))) || prctile(dist2o(:),99)-tol_frame > 0 || prctile(dist2o(:),99)-tol_frame< -err)  &&  ...
        ...
        for dm = 1:no_dims
            ...
            first_notAccnan = find(~isnan(acc(:,ff,dm)),1,'first');
            try
                ppaccx = smooth(squeeze(acc(first_notAccnan:end,ff,dm)),tol,'moving');
            catch
                keyboard
            end
            acc2(first_notAccnan:end,dm) = ppaccx;
            acc2(1:first_notAccnan-1,:)=0; % Zero so it does not interfere with cumsum.
...
%         keyboard
        ...
        % Correction
%         corr =
%         nanmean(squeeze(trajectories(:,ff,:))-(vel2tr));
        ...
        if ~isempty(lowID)
            ...
            if tol == tolDeltaArray(lowID,1) || tol == tolDeltaArray(highID,1)
                converged = true;
    ...
    disp(['Final window width ' num2str(tolPrev)])
