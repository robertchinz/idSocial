function output=idSocial_auxiliaries_reduceCellArrayLoadOnTheFly(orig_cell,plot_mode,orig_cell4statistics,extra_dim_names)
...
% Needed from orig_cell:
% cellfun(@(x) size(x),orig_cell,'UniformOutput',false);
% ndims(orig_cell);
% size(orig_cell);
...
dim_names_orig={'Group'; 'Day'; 'Trial'; 'Time'; 'Focal'; 'Neighbor';  'EdgeX'; 'EdgeY'; 'EdgeZ'};
dim_names_short={'Gr'; 'Day'; 'Tr'; 'Tm'; 'Foc'; 'Nb';  'EdX'; 'EdY'; 'EdZ'};
...
dayidx=find(strcmpi(dim_names,'day'));


if isfield(plot_mode,'reorder') && ~isempty(plot_mode.reorder) % For reordering using body length
...
if ~isempty(data_filter) && iscell(data_filter) && size(data_filter,1)==1
    ...
    % focals 1 and 2
    select_combs = combvec(gr_idces,day_idces,tr_idces,tm_idces,foc_idces,nb_idces);
...
elseif ~isempty(data_filter) && iscell(data_filter) && size(data_filter,1)>1
    % Each row contains seperate filters. Add possible combinations of
    % 'free' indices
    all_combs = combvec(gr_idces,day_idces,tr_idces,tm_idces,foc_idces,nb_idces);
                ...
                switch filter_idx{pidx,lc}
                    ...
                    case dayidx
                        ...
                        if max(day_idces)>size(orig_cell,dayidx)
                            warning([mfilename ': WrongIndex'],[mfilename ': Given day index exceeds array size.']);
...
% if strcmpi(plot_mode.xaxis,'EdgesX') && isfield(plot_mode,'edges') && ~isempty(plot_mode.edges)
%     if iscell(plot_mode.edges)
%         output.ticks{1} = plot_mode.edges{1};
%     else
%         output.ticks = plot_mode.edges;
%     end
% end
% if strcmpi(plot_mode.yaxis,'EdgesY') && isfield(plot_mode,'edges') && ~isempty(plot_mode.edges) 
%     if iscell(plot_mode.edges) && numel(plot_mode.edges)>1
%        output.ticks{2} = plot_mode.edges{2};
%     end
% end
% if strcmpi(plot_mode.xaxis,'Time') && isfield(plot_mode,'time_intervals_min') && ~isempty(plot_mode.time_intervals_min)
%     output.ticks = (0:xaxis_length-1)*time_intervals_min/2;
% end

% catch
%     warning([mfilename ': Failed to prepare data for presentation.'])
% end
end
