function input_data=idSocial_auxiliaries_signDiffTimeMaps(input_data,visible)
...
temp=arrayfun(@(x)x.data.(field),input_data,'uniformoutput',false);

meantimemap_alltrials_allfish=cell(no_groups,1);
C2=cell(size(temp{1,1},1),size(temp{1,1},2),no_groups);
for group=1:no_groups
    for trial=1:no_trials
        temp2=cat(3,temp{group,:});
        meantimemap_alltrials_allfish{group}=nanmean(temp2,3);
...
for group1=2:no_groups+1
    for group2=group1+1:no_groups+1
        mapcell{group1,group2}= meantimemap_alltrials_allfish{group1-1}-meantimemap_alltrials_allfish{group2-1};
    ...
    mapcell{1,group1}=meantimemap_alltrials_allfish{group1-1};
    mapcell{group1,1}=meantimemap_alltrials_allfish{group1-1};
end
%%
gl = opengl('data');
...
if min(input_data(group,1).options.edges{1})<0
    noticsx=ceil(max(input_data(group,1).options.edges{1})*2);
else
    noticsx=ceil(max(input_data(group,1).options.edges{1}));
end
if min(input_data(group,1).options.edges{2})<0
    noticsy=ceil(max(input_data(group,1).options.edges{2})*2);
else
    noticsy=ceil(max(input_data(group,1).options.edges{2}));
end

% if  ~isempty(strfind(lower(input_data(1,1).options.heatmapstring{1}),'angle'))
%     ticsx=round(input_data(1,1).options.edges{1}(1)):round(input_data(1,1).options.edges{1}(end));
%     ticsx=round((ticsx-input_data(1,1).options.edges{1}(1))*size(meantimemap_alltrials_allfish{1},2)/(input_data(1,1).options.edges{1}(end)-input_data(1,1).options.edges{1}(1)));
%     ticsx(1)=1;
% else
%     ticsx=input_data(1,1).options.edges{1}(round(input_data(1,1).options.edges{1})==input_data(1,1).options.edges{1});
%     ticsx=round(0:size(meantimemap_alltrials_allfish{1},2)/noticsx:size(meantimemap_alltrials_allfish{1},2));
%     ticsx(1)=1;
%
%     %     ticsx=ticsx(2:end-1);
%     %     ticsx=ticsx(2:end-1);
%
% end
% if  size(input_data(1,1).options.heatmapstring,1)==1 && ~isempty(strfind(lower(input_data(1,1).options.heatmapstring{1}),'angle')) || size(input_data(1,1).options.heatmapstring,1)==2 &&~isempty(strfind(lower(input_data(1,1).options.heatmapstring{2}),'angle'))
%     ticsy=round(input_data(1,1).options.edges{2}(1)):round(input_data(1,1).options.edges{2}(end));
%     ytic=round((ticsy-input_data(1,1).options.edges{2}(1))*size(meantimemap_alltrials_allfish{11},1)/(input_data(1,1).options.edges{2}(end)-input_data(1,1).options.edges{2}(1)));
%     ytic(1)=1;
% else
%     ticsy=input_data(1,1).options.edges{2}(round(input_data(1,1).options.edges{2})==input_data(1,1).options.edges{2});
%     ytic=round(0:size(meantimemap_alltrials_allfish{1},1)/noticsy:size(meantimemap_alltrials_allfish{1},1));
%     ytic(1)=1;
%     %     ytic=ytic(2:end-1);
%     %     ticsy=ticsy(2:end-1);
% end

% if  ~isempty(strfind(lower(input_data(group,1).options.heatmapstring{1}),'angle'))
%     ticsx=round(input_data(group,1).options.edges{1}(1)):round(input_data(group,1).options.edges{1}(end));
%     ticsx=round((ticsx-input_data(group,1).options.edges{1}(1))*size(meantimemap_alltrials_allfish{1},2)/(input_data(group,1).options.edges{1}(end)-input_data(group,1).options.edges{1}(1)));
%     ticsx(1)=1;
% else
%     %         ticsx=input_data(group,1).options.edges{1}(round(input_data(group,1).options.edges{1})==input_data(group,1).options.edges{1});
%     % %         ticsx=round(0:size(meantimemap_alltrials_allfish{1},2)/noticsx:size(meantimemap_alltrials_allfish{1},2));
%     %         ticsx=round(0:size(meantimemap_alltrials_allfish{1},2)/noticsx:size(meantimemap_alltrials_allfish{1},2));
%     %
%     %         ticsx(1)=1;
...
%     msizex=size(meantimemap_alltrials_allfish{1},2);
%     ticsx=-floor(max(abs(edgesx))):floor(max(abs(edgesx)));
%     ticsx=(ticsx-edgesx(1))/msizex;
%     
%     %     ticsx=ticsx(2:end-1);
%     %     ticsx=ticsx(2:end-1);
%     
% end
% if  size(input_data(group,1).options.heatmapstring,1)==1 && ~isempty(strfind(lower(input_data(group,1).options.heatmapstring{1}),'angle')) || size(input_data(group,1).options.heatmapstring,1)==2 &&~isempty(strfind(lower(input_data(group,1).options.heatmapstring{2}),'angle'))
%     ticsy=round(input_data(group,1).options.edges{2}(1)):round(input_data(group,1).options.edges{2}(end));
%     ytic=round((ticsy-input_data(group,1).options.edges{2}(1))*size(meantimemap_alltrials_allfish{1},1)/(input_data(group,1).options.edges{2}(end)-input_data(group,1).options.edges{2}(1)));
%     ytic(1)=1;
% else
%     %         ticsy=input_data(group,1).options.edges{2}(round(input_data(group,1).options.edges{2})==input_data(group,1).options.edges{2});
%     %         ytic=round(0:size(meantimemap_alltrials_allfish{1},1)/noticsy:size(meantimemap_alltrials_allfish{1},1));
%     %         ytic(1)=1;
%     edgesy=input_data(group,1).options.edges{2};
%     msizey=size(meantimemap_alltrials_allfish{1},1);
%     ticsy=-floor(max(abs(edgesy))):floor(max(abs(edgesy)));
%     ytic=(ticsy-edgesy(1))/msizey;
%
%     %     ytic=ytic(2:end-1);
%     %     ticsy=ticsy(2:end-1);
% end
...
%
% if (min(maps(:)))<.001 && abs((max(maps(:)))-2*pi)<.001
%     color_limits=[0 2*pi];
%     act_colormap=jet;
% elseif abs(abs(min(maps(:)))-pi)<.001 && abs((max(maps(:)))-pi)<.001
%     color_limits=[-pi pi];
%     act_colormap=jet;
% elseif min(maps(:))<0;
...
clim=-inf;
temp=cat(3,meantimemap_alltrials_allfish{:});
...
%     clim=-inf;
%     temp=cat(3,meantimemap_alltrials_allfish{:});
%     clim= max(clim,max(abs(prctile(temp(:),pos_uppercut)),abs(prctile(temp(:),100-pos_uppercut))));
%
%     temp=cat(3,mapcell{:});
%     clim= max(clim,max(abs(prctile(temp(:),pos_uppercut)),abs(prctile(temp(:),100-pos_uppercut))));
%
...
% for group1=1:size(mapcell,1)
%     for group2=1:size(mapcell,2)
%         if size(mapcell{group1,group2},1)==1 && size(mapcell{group1,group2},2)==1 && ishandle(mapcell{group1,group2})
%             hctr=findobj([mapcell{group1,group2}],'-regexp','Tag','_contour_*');
%             if ~isempty(hctr)
    ...
    for group2=no_groups+1:-1:1
        ...
        if ~isempty(mapcell{group1,group2})
            ...
            elseif group2==1 && group1>1
                ...
                hact=gca;
                set(gca,'YDir','normal','Xtick',ticsx,'Ytick',ticsy','FontSize',axislabelsize);
            ...
            elseif group2>1 && group1==1
                ...
                %             keyboard
                %         end
                %                 imagesc(edgesx,edgesy,meantimemap_alltrials_allfish{group2-1},color_limits);
                imagesc(edgesx,edgesy,((map_act-clims_all(group1,group2,1))/(clims_all(group1,group2,2)-clims_all(group1,group2,1)))*255+1+(cmap_subplot(group1,group2)-1)*256+1);
                set(gca,'CLim', [0 max(cmap_subplot(:))*256])
                colormap(cmap_all)
                hact=gca;
            ...
            if group1==1 && group2==cols_subplot && group1<rows_subplot
                ...
                if group2==cols_subplot-1 && cols_subplot==2 % If N=2 put y-label on both sides
                    B = axes('Position',get(hact,'Position'),'YLim',get(hact,'YLim'),'XLim',get(hact,'XLim'));
                    set(B,'yaxislocation','left','Xtick',ticsx,'Ytick',ticsx,'YtickLabel',ticsy,'FontSize',axislabelsize)
                    axis tight square
            ...
            if group1>1 && group2==cols_subplot && group1<rows_subplot || group1>1 && group2==cols_subplot-1 && group1==rows_subplot
                ...
                if group2==cols_subplot-1 && cols_subplot==2 % If N=2 put y-label on both sides
                    B = axes('Position',get(hact,'Position'),'YLim',get(hact,'YLim'),'XLim',get(hact,'XLim'));
                    set(B,'yaxislocation','left','Xtick',ticsx,'Ytick',ticsx,'YtickLabel',ticsx,'FontSize',axislabelsize)
                    axis tight square
                    ylabel(B, input_data(group,1).options.plot_axislabelstring{2},'fontsize',rowlabelfontsize)
                    uistack(gca,'bottom')
                end
