function [turningdistribution, turning_median,turning_mode, turning_ratio,inbins, turning,funcinfo]=idSocial_turningReactionDistribution(tr,edges,angle_range,framerate,bodylength,normalization,method,kds_bandwidth,collapse_left_right,xIsDistance,arena_center)
% Calculates turning acceleration (component perpendicular to focal's
...
for ff=1:no_fish
    for nf=1:no_fish
        if ff~=nf && ~rand_check(ff,nf)
            ...
            elseif strcmpi(xIsDistance,'turning')
                distance_vector = val;
            end
            ...
            turning_median(ff,nf)=nanmedian(ttt);
            
            turning_circvar(ff,nf)=idSocial_circ_var(ttt);
            turning_ratio(ff,nf,1:numel(ttt))=ttt;
...
if strcmpi(xIsDistance,'distance_from_center') && no_fishOrig==1
    turningdistribution = turningdistribution(1,2,:);
    inbins = inbins(1,2,:);
    turning_median = turning_median(1,2,:);
    turning_mode = turning_mode(1,2,:);
    turning_ratio = turning_ratio(1,2,:);
    turning = turning(1,2,:);
end