function [turningReact, turningReactInBins, turningReactCell,turningReactOneBin]=idSocial_turningReaction(tr,edges,framerate,bodylength,normalize_by_individual_max,rotate_z,method,collapse)
...
for f1=1:no_fish
    for f2=1:no_fish
        if f1~=f2  && ~rand_check(ff,nf)
            ...
            if collapse
                d = sqrt(sum(focal_to_neighbour_vector_rotated(:,f2,f1,1:2).^2,4))/bodylength;
                 map_vector(distance_vector<0) = -  map_vector(distance_vector<0);
%                d(distance_vector<0) = -d(distance_vector<0);
               distance_vector = d';
            end
