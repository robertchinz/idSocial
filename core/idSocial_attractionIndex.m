function varargout=idSocial_attractionIndex(trajectory,edges,spacing,framerate,bodylength,discard_percentage_outliers,rotate_z,method)
...
if nargout==2*no_maps+2+1
    maps_allframes=NaN(no_fish,no_fish,1,no_frames,heatmapsize(1),heatmapsize(2));
end
...
for focal=1:no_fish
    for neighbour=1:no_fish
        if focal~=neighbour
            ...
            mapval(2,:)=sum(squeeze(acc_rot_norm(:,focal,focal,1:2))'.*squeeze(foc_to_nb_vec_dir_rot(:,neighbour,focal,1:2))',1);
        ...
        for ff=1:size(indices_in_bin,1)
            for nf=1:size(indices_in_bin,2)
                if ff~=nf
                    ...
                    mapval(2,:)=sum(squeeze(acc_rot_norm(:,ff,ff,1:2))'.*squeeze(foc_to_nb_vec_dir_rot(:,nf,ff,1:2))',1);
                    mapval_new=mapval;
