function varargout=idSocial_dynamicMaps(trajectory,edges,spacing,framerate,bodylength,discard_percentage_outliers,rotate_z,method,normalization,collapse)
...
foc_to_nb_dir=foc_to_nb_vec./repmat(foc_to_nb_distance,[1,1,1,no_dim]);
    ...
    for nf=1:no_fish
        if ff~=nf && ~rand_check(ff,nf)
            ang_foc_vel_norm_z_plane_and_nb_pos(:,ff,nf)=...
                mod(atan2(foc_to_nb_dir(:,nf,ff,1).*vel_norm(:,ff,ff,no_dim)-vel_norm(:,ff,ff,1).*foc_to_nb_dir(:,nf,ff,no_dim),vel_norm(:,ff,ff,1).*foc_to_nb_dir(:,nf,ff,1)+vel_norm(:,ff,ff,no_dim).*foc_to_nb_dir(:,nf,ff,no_dim)),2*pi);
        end
    end
end
% angle_focal_movement_direction_and_neighbour_position=ang_foc_vel_norm_z_plane_and_nb_pos;

% if iscell(trajectory) && isstruct(trajectory{1}) % from splines
%     no_dim=2;
%     [trajectory, vel, acc] = idSocial_auxiliaries_spline2arrays(trajectory);
%     no_fish=size(trajectory,2);
%     no_frames=size(trajectory,1);
%     acc2=zeros(no_frames,no_fish,no_fish,3);
%     vel2=zeros(no_frames,no_fish,no_fish,3);
%     for ff=1:no_fish
%         acc2(:,ff,:,1:size(vel,3))=repmat(acc(:,ff,:),[1 no_fish 1]);
%         vel2(:,ff,:,1:size(vel,3))=repmat(vel(:,ff,:),[1 no_fish 1]);
%
%     end
%     acc=acc2;
%     vel=vel2;
...
%             %             if ~collapse
%             %                 ang_foc_vel_norm_z_plane_and_nb_pos(:,ff,nf)=...
%             %                     mod(atan2(foc_to_nb_dir(:,nf,ff,2).*vel_norm(:,ff,ff,1)-vel_norm(:,ff,ff,2).*foc_to_nb_dir(:,nf,ff,1),vel_norm(:,ff,ff,1).*foc_to_nb_dir(:,nf,ff,1)+vel_norm(:,ff,ff,2).*foc_to_nb_dir(:,nf,ff,2)),2*pi);
%             ang_foc_vel_norm_z_plane_and_nb_pos(:,ff,nf)=...
...
%         else
%             %                 ang_foc_vel_norm_z_plane_and_nb_pos(:,ff,nf)=...
%             %                     acos(sum(foc_to_nb_dir(:,nf,ff,:).*vel_norm(:,ff,1,:),4));
%             %             end
%         end
%         ang_foc_vel_norm_z_plane_t1_and_t2(:,ff,nf)=...
%             vertcat(atan2(vel_norm(2:end,ff,nf,2).*vel_norm(1:end-1,ff,nf,1)-vel_norm(1:end-1,ff,nf,2).*vel_norm(2:end,ff,nf,1),vel_norm(2:end,ff,nf,1).*vel_norm(1:end-1,ff,nf,1)+vel_norm(2:end,ff,nf,2).*vel_norm(1:end-1,ff,nf,2)), NaN);
%     end
% end
% angle_focal_movement_direction_and_neighbour_position=ang_foc_vel_norm_z_plane_and_nb_pos;
% 
...
%     mod(atan2(foc_to_nb_vec_rot(:,:,:,2),foc_to_nb_vec_rot(:,:,:,1)),2*pi);
%%
...
for focal=1:no_fish
    for neighbour=1:no_fish
        if focal~=neighbour
            ...
            if collapse
                neg_x = coordinates(:,1)<0;
                coordinates(neg_x,1) = -coordinates(neg_x,1);
                mapval(1,neg_x) = -mapval(1,neg_x);
                mapval(3,neg_x) = -mapval(3,neg_x);
            end
