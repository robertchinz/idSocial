function input_data=idSocial_dynamics_PositionMapPolar(input_data,options,plot_mode)
...
if nargin >= 1
    plot_mode_def.statistics = {'Sum','Mean','Median','Pool'};
    plot_mode_def.display_mode='MapPolar';
    plot_mode_def.xaxis={'EdgeX'};
    plot_mode_def.yaxis={'EdgeY'};
    plot_mode_def.xlabel = 'Left-Right';
    plot_mode_def.ylabel = 'Behind-Front';
    plot_mode_def.normalization={'None','No_Frames','Density','Density_Effective'};
...
% if isfield(options_new,'normalization') && ~isempty(options_new.normalization) && strcmpi(options_new.normalization,'none')
%     plot_mode_def.statistics={'sum'};
% elseif isfield(options_new,'normalization') && ~isempty(options_new.normalization) && strcmpi(options_new.normalization,'density')
