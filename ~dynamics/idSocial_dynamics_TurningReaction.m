function input_data=idSocial_dynamics_TurningReaction(input_data,options,plot_mode)
% Calculates the turning strength of a focal individual from its component
% of the acceleration vector perpendicular to the focal's direction of 
% movmement. The turning strength can be plotted
...
def_options(1).xaxis = {'turning', 'distance', 'left_right','distance_from_center'};
...
if nargin >= 1
    if (isfield(opt(1),'edges') && ~isempty(opt(1).edges) && all(isfinite(opt(1).edges))) || (isfield(opt(1),'limits_min') && ~isempty(opt(1).limits_min) && isfinite(opt(1).limits_min) && ...
        ...
        if isfield(opt(1),'xaxis') && strcmpi(opt(1).xaxis,'turning')
        ...
        plot_mode_def.extraDims = {'Focal','Neighbor','Turning'};
    else
        plot_mode_def.statistics = {'Positive_Ratio','Mean','Median','Pool'};
        plot_mode_def.display_mode='plot2d';
        def_options(1).act_method='dynamics_TurningReaction4Statistics';
        plot_mode_def.label = 'Turning';
...
if strcmpi(options_new.xaxis,'distance')
    ...
    statString = 'dynamics_TurningReaction4Statistics';
end
...
functionInfo.handle=@idSocial_turningReactionDistribution;
...
%     '';...
%     '';...
%     'dynamics_TurningReaction';...
%     'dynamics_TurningReaction4Statistics';...
%     'dynamics_TurningReactionInfo';...
%     };
